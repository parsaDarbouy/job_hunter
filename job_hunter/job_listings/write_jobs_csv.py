...
def write_jobs_csv(postings: list[JobPosting], output_path: Path) -> None:
    """
    Persist ``postings`` with columns ``url``, ``job_title``, ``location``, ``company_name``.
    ...
    with output_path.open("w", encoding="utf-8", newline="") as handle:
        ...
        writer.writerow(["url", "job_title", "location", "company_name"])
        for posting in postings:
            writer.writerow([posting.url, posting.title, posting.location, posting.company_name])
