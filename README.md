...
**Filters today:** title allow / block lists, geography rules from `location_constraints`, and **seniority** from `acceptable_seniority_levels` / `not_acceptable_seniority_levels`. Seniority is inferred from **job title keywords** (listing APIs rarely expose a structured level). Titles with no seniority signal still pass when an allow-list is set. Compensation ranges and stated YOE in `position.yaml` are captured in `criteria_snapshot` for transparency; compensation is not used to drop rows automatically when listings do not include structured pay.

**Optional Gemini location rescue:** Under `location_constraints.gemini_description_rescue`, set `enabled: true` to re-check rows that fail the ATS **location** string heuristic. The exporter sends the flattened job description plus your geography bullets to **Gemini CLI** (same `gemini auth` workflow as resume ingest); if the model’s `likelihood` is at least `minimum_likelihood` (default `0.5`), the row is kept. Requires enough plain text (`min_plain_text_chars`, default `80`) from bundled Greenhouse/Ashby/Workable description HTML when the API supplies it. Use `listings:export --gemini-binary` / `--gemini-model` to override binaries or model aliases.
